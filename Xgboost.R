...
#读取输入文件
data=read.table("内质网应激_COX_GeneExp.txt" , header=T, sep="\t", check.names=F, row.names=1)
data=t(data)
...
df <- data.frame(data)
df$group <- as.factor(group)  # 将分组信息转换为因子变量

# 准备训练数据
X <- df[, -ncol(df)]  # 去掉最后一列分组信息
y <- df$group
...
write.table(feature_importance$Feature, file="xgboost.gene.txt", sep="\t", quote=F, row.names=F, col.names=F)
...
p1 <- ggplot(data = feature_importance, aes(y = reorder(Feature, Gain), x = Gain, fill = Gain)) +
  ...
  geom_text(aes(label = "0.01"), x = 0.01, y = Inf, vjust = -0.5, hjust = 0, color = "black") +  # 添加文本标签
